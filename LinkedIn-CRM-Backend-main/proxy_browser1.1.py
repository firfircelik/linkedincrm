...
def get_accounts():
    cursor, conn = get_db()
    # Use the LIKE operator with '%C3%' to find names containing 'C3'
    cursor.execute("""SELECT name, proxyip, proxyport, proxyuser, proxypass, cookies FROM "CRM_account";""")
...
def open_browser_with_proxy(name, proxyip, proxyport, proxyuser, proxypass, cookies):
    threading.Thread(target=_open_browser, args=(name, proxyip, proxyport, proxyuser, proxypass, cookies)).start()

def _open_browser(account_name, proxyip, proxyport, proxyuser, proxypass, cookies):
...
class DesktopApp:
    ...
    def create_account_frame(self, parent, account):
        name, proxyip, proxyport, proxyuser, proxypass, cookies = account
        ...
        open_button = ttk.Button(button_frame, text="Open", command=lambda: open_browser_with_proxy(name, proxyip, proxyport, proxyuser, proxypass, cookies))
